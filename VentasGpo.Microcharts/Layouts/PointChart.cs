...
namespace Microcharts
{
    ...
    public class PointChart : Chart
    {
        ...
        {
            //Se Cambio a representarse en la parte inferior del canvas
            var py = Convert.ToSingle(height) - headerHeight + this.Margin;
            this.DrawLabels(canvas,
                            labels,
                            points.Select(p => new SKPoint(p.X, py)).ToArray(),
                            //points.Select(p => new SKPoint(p.X, headerHeight - this.Margin)).ToArray(),
                            labelSizes,
                            this.Entries.Select(x => x.TextColor.WithAlpha((byte)(255 * this.AnimationProgress))).ToArray(),
        ...
        protected void DrawFooter(SKCanvas canvas, string[] labels, SKRect[] labelSizes, SKPoint[] points, SKSize itemSize, int height, float footerHeight)
        {
            this.DrawLabels(canvas,
                            ...
                            labelSizes,
                            this.Entries.Select(x => x.TextColor).ToArray(),
                            this.ValueLabelOrientation,
                            false,
                            itemSize,
                            height);
